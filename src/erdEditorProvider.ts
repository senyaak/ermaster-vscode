...
export class ErdEditorProvider implements vscode.CustomTextEditorProvider {
  static readonly viewType = 'erdesigner.erdEditor';
  ...
  static register(context: vscode.ExtensionContext): vscode.Disposable {
    ErdEditorProvider.diagnostics = vscode.languages.createDiagnosticCollection('erdesigner');
...
export function openDdlForText(text: string): void {
  ...
  try {
    ...
    vscode.window.showErrorMessage(`ER Designer: cannot parse diagram — ${String(e)}`);
    return;
  }
...
async function saveExport(document: vscode.TextDocument, ext: 'svg' | 'png', data: Buffer): Promise<void> {
  ...
  vscode.window.showInformationMessage(`ER Designer: exported ${target.path.split('/').pop()}`);
}
